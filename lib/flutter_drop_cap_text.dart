...
///
/// This package provides the [DropCapText] widget, which displays text
/// with a stylized drop cap (initial letter) effect.
library;
...
class DropCapText extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final textSpan = TextSpan(
      ...
      style: textStyle.copyWith(
        fontSize: MediaQuery.textScalerOf(context).scale(textStyle.fontSize ?? 14),
      ),
    );
  ...
  Widget _buildBaseline(BuildContext context, TextStyle textStyle, TextStyle capStyle) {
...
class MarkdownParser {
  ...
  MarkdownParser(this.data) {
    ...
    void addSpan(String markup, {required bool isOpening}) {
      ...
      if (bold && markup != markupBold) markups.add(Markup(markupBold, isActive: true));
      if (italic && markup != markupItalic) markups.add(Markup(markupItalic, isActive: true));
      if (underline && markup != markupUnderline) markups.add(Markup(markupUnderline, isActive: true));
  ...
  MarkdownParser subchars(int startIndex, [int? endIndex]) {
    ...
    for (var s = 0; s < spans.length; s++) {
      ...
      if (skip <= 0) {
        ...
        subspans.add(
          MarkdownSpan(style: span.style, markups: span.markups, text: span.text.substring(skip, span.text.length)),
        );
        skip = 0;
      }
    }
